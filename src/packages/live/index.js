...
const baseUrl = "https://live.shopee.com.my";
...
const getChatroom = ({ token }) => {
  ...
  return axios
    .get(
      `https://chatroom-live.shopee.com.my/api/v1/fetch/chatroom/${id}/message?${qs.stringify(
        query
      )}`,
      { headers: { Cookie: cookies.join(";") } }
    )
...
const connectWs = ({ sessId, uid, deviceId, userSig }) => {
  ...
  return axios
    .get(`https://live.shopee.com.my/im/v1/comet?${qs.stringify(query)}`, {
      headers: {
